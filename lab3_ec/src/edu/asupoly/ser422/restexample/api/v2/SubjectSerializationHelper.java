...
public final class SubjectSerializationHelper {
	...
	public String generateJSON(Subject subject,String self, String base) throws JsonProcessingException {
		try {
			ObjectMapper om = new HALMapper();
			subject.setSelf(new HALLink.Builder(self).build());
			subject.setSelfLink(self);
			subject.setBookRel(new HALLink.Builder(base+"v2/books").build());
			subject.setAuthorRel(new HALLink.Builder(base+"v2/authors").build());
			subject.setSubjectBookRel(__bService.findBooksBySubject(subject.getSubjectId(), base));
			subject.setSubjectAuthorRel(__bService.getAuthorsBySubjectID(subject.getSubjectId(), base));
			return om.writeValueAsString(subject);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
